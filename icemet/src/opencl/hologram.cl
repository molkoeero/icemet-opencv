...
)
{
	...
	if (x >= src_w || y >= src_h) {
		...
		// e^(i * pi * (x+y))
		cfloat shift = cexp(cmul(cnum(M_PI * (x+y), 0.0), cnum(0.0, 1.0)));
		dst[y*dst_w + x] = cmul(cnum(src[y*src_w + x], 0.0), shift);
	}
}
...
)
{
	...
	// e^(-i * pi * (x+y))
	cfloat shift = cexp(cmul(cnum(-1 * M_PI * (x+y), 0.0), cnum(0.0, 1.0)));
	dst[y*dst_w + x] = cabs(cmul(src[y*src_w + x], shift));
}
...
)
{
	...
	// e^(-i * pi * (x+y))
	cfloat shift = cexp(cmul(cnum(-1 * M_PI * (x+y), 0.0), cnum(0.0, 1.0)));
	uchar val = cabs(cmul(src[y*src_w + x], shift));
...
)
{
	...
	float u = ((float)x - (float)w/2.0 - 1.0) / size.x;
	float v = ((float)y - (float)h/2.0 - 1.0) / size.y;
